...
export function createSecureErrorResponse(
  ...
  if (process.env.NODE_ENV === 'development' && context?.originalError) {
    response.debug = sanitizeErrorForLogging(context.originalError);
  }
...
export function handleDatabaseError(
  ...
  if (error && typeof error === 'object' && 'code' in error) {
    ...
    } else if (errorCode === '23503') { // Foreign key constraint violation
